...
class Superpoint
{
    ...
    public:
        ...
        void extract_points(std::vector<std::vector<float>> &kps, zdl::DlSystem::ITensor *outputHeatmap, const float threshold, const int &hm_height, const int &hm_width, const int &hm_channel, std::vector<std::vector<float>> &reshape_hm);
        void nms(std::vector<std::vector<float>> &kps, std::vector<std::vector<float>> &nms_kps, const int &nms_radius);
        void extract_descriptor(zdl::DlSystem::ITensor *outputDesc, const std::vector<cv::Point2f> &final_kps, std::vector<std::vector<float>> &descriptor, const int &desc_height, const int &desc_width, const int &desc_channel, const int &hm_channel);
