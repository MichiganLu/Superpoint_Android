...
//to do: deal with no keypoint case
int main()
{
    // load model
    ncnn::Net superpoint;
    superpoint.opt.use_vulkan_compute = false;
    if (superpoint.load_param("/home/cvte-vm/Deep_Feature_Extract/pytorch-superpoint/implementation/coco_pretrained.param"))
        exit(-1);
    if (superpoint.load_model("/home/cvte-vm/Deep_Feature_Extract/pytorch-superpoint/implementation/coco_pretrained.bin"))
        exit(-1);

    // load image
    const char* inputFile = "/media/cvte-vm/C4CE54D9CE54C4F8/3D_Datasets/HPatches/hpatches-sequences-release/i_pool/1.ppm";
    ...
    //extract descriptor
    std::vector<std::vector<float>> descriptor;   //descriptor of dim [N, 256], N for number of nmsed keypoints, 256 for descriptor length
    extract_descriptor(out2, nms_kps, descriptor);

    //find subpixel position of kps
    std::vector<cv::Point2f> final_kps;
